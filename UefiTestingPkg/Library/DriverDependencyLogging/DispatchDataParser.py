...
class EvaluationReporting:
    ...
    def __init__(self, ResultsFile):
        ...
        self.FileHandle.write(datetime.now().strftime(
            "%b %d, %Y - %I:%M:%S %p\n\n"))
...
def ReadPertinentData(InputFile, GuidNames):
    ...
    if BeginTagCount > EndTagCount:
        PertinentDataReadError("Begin tag '{}' is missing a matching end tag '{}'".format(
            DEPEX_LOG_BEGIN, DEPEX_LOG_END))
    if EndTagCount > BeginTagCount:
        PertinentDataReadError("End tag '{}' is missing a matching begin tag '{}'".format(
            DEPEX_LOG_END, DEPEX_LOG_BEGIN))
    ...
    for Line in Lines:
        ...
        if not IsHex(RawDepex):
            PertinentDataReadError(
                "Line depex is not a two-char per byte hex value", Line)
        i = 2
        ...
        for Guid in Parts:
            ...
            if Guid != "":
                if not GuidParser.is_guid_in_reg_format(Guid):
                    PertinentDataReadError(
                        "Guid ({}) is not properly formatted".format(Guid), Line)
                UsedProtocols.append({"Guid": Guid,
                                      "Name": NameFromGuidNameList(GuidNames, Guid)})
                # UsedProtocols.append(GuidEntryFromGuidString(Guid, GuidNames) })
...
def CollectGuidNames(foldersrc: str):
    from edk2toollib.uefi.edk2 import guid_list

    gl = guid_list.GuidList.guidlist_from_filesystem(
        foldersrc, ["/Build", "/Conf"])
    # sort this for later so that we can easily return the multiple name feature :)
    return sorted(gl, key=lambda x: x.guid.upper())
...
def NameFromGuidNameList(GuidNames, Guid):
    results = list(filter(lambda g: g.guid == Guid, GuidNames))
    if len(results) == 0:
        return "< unknown >"
    return " | ".join([x.name for x in results])
...
def CreateDepexStack(RawDepex, GuidNames):
    ...
    while len(DepexList) > 0:
        ...
        CommandName = DEPEX_CMD_ENCODING.get(
            Command, "ERROR: Invalid command byte")
        Guid = ""
        GuidName = ""
        ...
        if CommandName in DEPEX_CMDS_WITH_GUIDS:
            ...
            Guid = "{}-{}-{}-{}-{}".format("".join(GuidByteList[3::-1]),  # Part A - UINT32  - bytes [0:3] are reverse ordered
                                           # Part B - UINT16  - bytes [4:5] are reverse ordered
                                           "".join(GuidByteList[5:3:-1]),
                                           # Part C - UINT16  - bytes [6:7] are reverse ordered
                                           "".join(GuidByteList[7:5:-1]),
                                           # Part D - UINT8[] - bytes [8:9] are in correct order
                                           "".join(GuidByteList[8:10]),
...
Parser = argparse.ArgumentParser(
    description='Script to collect and report UEFI runtime dependency expression usage')
Parser.add_argument("--Input", required=True, action='store', metavar="<file>",
                    help='Name of input file to be scanned for dispatch data.  Can be a UEFI boot log or a file containing a dump of the data from variable services.')
Parser.add_argument("--Output", required=False, action='store', metavar="<dir>",
                    help='Path of a directory to receive all output files. (optional)')
Parser.add_argument("--UefiTree", required=False, action='store', metavar="<dir>",
                    help='Path to a directory containing UEFI code to scan and collect GUID names from DEC files (optional)')
...
error = None
for index in range(len(GuidNames)):
    i = GuidNames[index]
        ...
        if i.guid == previous.guid:  # Error
            if (error is None):
                ...
                error = [
                    f"    Guid: {i.guid}",
                    f"    FileName :: GuidName:"
                    f"    {previous.absfilepath} :: {previous.name}"
                ]
                WarnList.append(error)

            error.append(f"    {i.absfilepath} :: {i.name}")
        else:
...
for Entry in DispatchData:
    ...
    for ProtocolInfo in Entry["UsedProtocols"]:
        if GuidNotCoveredByDepex(Entry['DepexStack'], ProtocolInfo['Guid']):
            Lines.append("        {} ( {} )".format(
                ProtocolInfo['Guid'], ProtocolInfo['Name']))
    if len(Lines) > 0:
        WarnList.append([
...
if len(WarnList) > 0:
    Report.Warning([
        "Found {} driver(s) that used a protocol GUID not covered by its dependency expression".format(
            len(WarnList)),
...
for Entry in DispatchData:
    ...
    for StackEntry in Entry['DepexStack']:
        if GuidNotUsedInLocateProtocol(Entry['UsedProtocols'], StackEntry['Guid']):
            Lines.append("        {}  ( {} )".format(
                StackEntry['Guid'], StackEntry['GuidName']))
    if len(Lines) > 0:
        WarnList.append([
            "    FileName: {}".format(Entry['Name']),
...
if len(WarnList) > 0:
    Report.Warning([
        "Found {} driver(s) that declared a GUID in the depex without using it in a LocateProtocol call".format(
            len(WarnList)),
